...
public class GsmAlphabet {
     ...
     */
    public static int
    charToGsm(char c, boolean throwException)  {
        int ret;
    ...
    public static byte[] stringToGsm7BitPackedWithHeader(String data, byte[] header,  int languageTable, int languageShiftTable)           {
     ...
     */
    public static int
    countGsmSeptets(char c, boolean throwsException)  {
     ...
     */
    public static com.android.internal.telephony.GsmAlphabet.TextEncodingDetails
    countGsmSeptets(CharSequence s, boolean use7bitOnly) {
        ...
        if (sEnabledSingleShiftTables.length + sEnabledLockingShiftTables.length == 0) {
            com.android.internal.telephony.GsmAlphabet.TextEncodingDetails ted = new com.android.internal.telephony.GsmAlphabet.TextEncodingDetails();
            int septets = com.android.internal.telephony.GsmAlphabet.countGsmSeptetsUsingTables(s, use7bitOnly, 0, 0);
            if (septets == -1) {
                return null;
            }
            ted.codeUnitSize = com.android.internal.telephony.SmsConstants.ENCODING_7BIT;
            ted.codeUnitCount = septets;
            if (septets > com.android.internal.telephony.SmsConstants.MAX_USER_DATA_SEPTETS) {
                ted.msgCount = (septets + (com.android.internal.telephony.SmsConstants.MAX_USER_DATA_SEPTETS_WITH_HEADER - 1)) /
                        com.android.internal.telephony.SmsConstants.MAX_USER_DATA_SEPTETS_WITH_HEADER;
                ted.codeUnitsRemaining = (ted.msgCount *
                        com.android.internal.telephony.SmsConstants.MAX_USER_DATA_SEPTETS_WITH_HEADER) - septets;
            } else {
                ted.msgCount = 1;
                ted.codeUnitsRemaining = com.android.internal.telephony.SmsConstants.MAX_USER_DATA_SEPTETS - septets;
            }
            return ted;
        }

        int maxSingleShiftCode = sHighestEnabledSingleShiftCode;
        List<GsmAlphabet.LanguagePairCount> lpcList = new ArrayList<GsmAlphabet.LanguagePairCount>(
                sEnabledLockingShiftTables.length + 1);

        // Always add default GSM 7-bit alphabet table
        lpcList.add(new GsmAlphabet.LanguagePairCount(0));
        for (int i : sEnabledLockingShiftTables) {
            ...
            if (i != 0 && !sLanguageTables[i].isEmpty()) {
                lpcList.add(new GsmAlphabet.LanguagePairCount(i));
            }
        }
        ...
        for (int i = 0; i < sz && !lpcList.isEmpty(); i++) {
            ...
            for (GsmAlphabet.LanguagePairCount lpc : lpcList) {
        ...
        for (GsmAlphabet.LanguagePairCount lpc : lpcList) {
            for (int shiftTable = 0; shiftTable <= maxSingleShiftCode; shiftTable++) {
                ...
                if (septets + udhLength > com.android.internal.telephony.SmsConstants.MAX_USER_DATA_SEPTETS) {
                    if (udhLength == 0) {
                        udhLength = UDH_SEPTET_COST_LENGTH;
                    }
                    udhLength += UDH_SEPTET_COST_CONCATENATED_MESSAGE;
                    int septetsPerMessage = com.android.internal.telephony.SmsConstants.MAX_USER_DATA_SEPTETS - udhLength;
